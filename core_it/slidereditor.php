  ...
  <div id="slider">
  <p><strong>Slider di articoli</strong></p>
  <table id="tabmenu" style="display:none;">
  <tr>
  <td>Titolo dello slide:</td>
  <td>Contenuto dello slide:</td>
  <td>Url articolo,immagine e grandezza:</td>
  <td>Anteprima:</td>
  </tr>
  </table>
  <table id="slidenum">
  </table>
  <button OnClick="var n_slide=prompt('Che numero metto a questa slide? Se e\' la prima slide che crei metti 1.'); if(isNaN(n_slide) == false && n_slide != null && n_slide != ''){ n=n_slide;newslide(); if(document.getElementById('tabmenu').style.display=='none') { document.getElementById('tabmenu').style.display=''; } }else { alert('Devi inserire un numero!'); }">Nuova Slide</button>
  <script type="text/javascript">
   ...
   function newslide()
   {
	...
	if(document.getElementById('title'+n) == null)
	{
		...
		var tr=table.insertRow(table.rows.length);
		var td0=tr.insertCell(0);
		td0.innerHTML='<br/><button OnClick=\'$("#slider'+n+'").slideToggle(); if (s==false) { this.innerHTML="Slide '+n+' &darr;";s=true; } else { this.innerHTML="Slide '+n+' &uarr;";s=false; } \'>Slide '+n+' &uarr;</button> <button OnClick="if(confirm(\'Sei sicuro di aggiungere questo slide?\') == true) { addslide('+n+'); }">Conferma slide</button><br/>';
	}
	else
	{
		alert('Questa slied gia\' esiste!');
	}
  }
  ...
  function addslide(slide)
  {
	...
	{
		if(confirm('Questo slide esiste gia\', sovrascriverlo?') == true)
		{
  ...
  <p><strong>Risultato del codice</strong></p>
...
<p>Copia l'intero codice e aggiungilo in una sidebar (ad es. nel messaggio di benvenuto).</p>